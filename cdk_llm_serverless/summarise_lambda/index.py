...
bedrock_runtime = boto3.client("bedrock-runtime", "eu-central-1")
...
def handler(event, context):
    ...
    if "-transcript.json" not in key:
        print("This demo only works with *-transcript.json. Got %s", key)
        return
    ...
    try:
        ...
        s3_client.put_object(
            Bucket=bucket,
            Key=f"summaries/{os.path.splitext(key)[0]}_summary.json",
            Body=summary,
